...
def construct_dataframe(res, exp_name, i2connectomeid):
    '''Take spike time dict and collect spikes in pandas dataframe
    ...
    i2connectomeid : dict
        Mapping between Brian IDs and connectome IDs
    ...
    d = {
        't': ts,
        'trial': nrun,
        'connectome_id': [i2connectomeid[i] for i in ids],
        'exp_name': exp_name
    }
    df = pd.DataFrame(d)
...
def run_exp(exp_name, neu_exc, path_res, path_comp, path_con,
    ...
    ----------
        exp_name: str
            name of the experiment
        neu_exc: list
            contains custom names or connectome IDs of neurons to be excited
        path_res: str
        ...
        neu_slnc : list (optional)
            contains custom names or connectome IDs of neurons to be silenced
        neu_exc2 : list (optional)
            contains custom names or connectome IDs of neurons to be excited with different frequency
    ...
    connectomeid2i = {j: i for i, j in enumerate(df_comp.index)}  # connectome ID: brian ID
    i2connectomeid = {j: i for i, j in connectomeid2i.items()} # brian ID: connectome ID
    ...
    with parallel_backend('loky', n_jobs=n_proc):
        exc = [ connectomeid2i[n] for n in neu_exc ]
        exc2 = [ connectomeid2i[n] for n in neu_exc2 ]
        slnc = [ connectomeid2i[n] for n in neu_slnc ]
        res = Parallel()(
            delayed(
    ...
    df = construct_dataframe(res, exp_name, i2connectomeid)
